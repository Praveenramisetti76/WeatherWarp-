...
def get_forecast(city):
    ...
    if response.status_code == 200:
        ...
        for entry in data["list"]:
            ...
            dt_ist = dt_utc.astimezone(IST).strftime("%Y-%m-%d %H:%M:%S")
...
def live_weather_update(cities):
    print("Weather updated at:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    cities_data = []
    for city in cities:
...
def schedule_weather_updates(cities):
    schedule.every(2).minutes.do(live_weather_update, cities=cities)
...
if __name__ == "__main__":
    cities = ["Hyderabad", "Nuzvid", "London", "Tokyo", "Delhi", "Vijayawada", "kavali", "Antarctica", "Kadapa" , "Mumbai" , "Goa"]
